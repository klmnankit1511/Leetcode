...
class Solution:
    def twoEggDrop(self, n: int) -> int:
        dp = [[-1 for _ in range(3)]for _ in range(n+10)]
        def utilsTwoEggDrop(n: int, k: int, dp: List[List[int]]) -> int:
            if dp[n][k] != -1:
                return dp[n][k]
            if k == 1 or n <= 1:
                return n
            ...
            for i in range(1,n+1):
                minn = min(minn,max(utilsTwoEggDrop(n-i,k,dp),utilsTwoEggDrop(i-1,k-1,dp))+1)
            dp[n][k] = minn
            return dp[n][k]
        k = 2
        return utilsTwoEggDrop(n,k,dp)
        
sol = Solution()
ans = sol.twoEggDrop(5)
print(ans)
